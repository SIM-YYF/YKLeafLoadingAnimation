...
public class LeafLoadingView extends View {
    ...
    public LeafLoadingView(Context context, AttributeSet attrs) {
        ...
        mLeafFloatTime = LEAF_FLOAT_TIME;//叶子开始时间
        mLeafRotateTime = LEAF_ROTATE_TIME;//叶子的旋转时间
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        //根据进度条框的宽和高，初始leaf_kuang矩形
        mOuterSrcRect = new Rect(0, 0, mOuterWidth, mOuterHeight);

        //根据整个view的框和高，初始view矩形
        mOuterDestRect = new Rect(0, 0, mTotalWidth, mTotalHeight);

        //进度条进度白色区域矩形
        mWhiteRectF = new RectF(
                mLeftMargin + mCurrentProgressPosition, //left
                mLeftMargin, //top
                mTotalWidth - mRightMargin,//right
                mTotalHeight - mLeftMargin//bottom
        );
        //进度条进度橘色区域矩形
        mOrangeRectF = new RectF(
                mLeftMargin + mArcRadius,//left
                mLeftMargin,//top
                mCurrentProgressPosition,//right
                mTotalHeight - mLeftMargin//bottom
        );
        ...
        mArcRectF = new RectF(
                mLeftMargin, //left
                mLeftMargin,//top
                mLeftMargin + 2 * mArcRadius,//right
                mTotalHeight - mLeftMargin//bottom
        );
